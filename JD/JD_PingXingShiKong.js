...
function Run(LauchAPPName, IsSeparation, IsInvite, IsJoinMember) {
  ...
  while (true) {
    ...
    let taskButtons = textMatches(/.*浏览.*s.*|.*浏览.*秒.*|.*去手Q频道.*|.*去下游参加游戏.*|.*累计浏览.*|.*浏览加购.*|.*浏览即可得.*|.*浏览并关注.*|.*逛会场可得.*|.*浏览可得.*|.*预约并了解.*|.*成功入会.*|.*小程序.*|.*去组队可得.*|.*每日6-9点打卡可得.*|.*去APP.*|.*参与城城点击.*|.*品牌墙店铺.*|.*玩AR游戏可得.*金币.*/).find()
    ...
    function timeTask() {
      ...
      if (textEndsWith("个商品领1000次元币").findOne(3000) != null) {//当前页浏览加购个商品领1000次元币|当前页点击浏览个商品领1000次元币
    ...
    function itemTask(cart) {
      ...
      textEndsWith("个商品领1000次元币").waitFor();//当前页浏览加购个商品领1000次元币|当前页点击浏览个商品领1000次元币
      if (!textContains('.jpg!q70').exists()) {
        console.log("模式2");
        for (var i = 0; i < 5; i++) {
          let Model2items = textEndsWith("个商品领1000次元币").findOne();
    ...
    if (taskText.match(/浏览.*s|浏览.*秒/)) {
      ...
      console.log("领取成功");
      if (!text("累计任务奖励").exists() && (text("赚次元币赢红包").exists())) {
    ...
    for (var i = 0; !text("累计任务奖励").exists(); i++) {
      ...
      if(text("已发放至“京东APP-我的-优惠券”，可立即使用").exists()){
        ...
        if(text("健康研究室").exists() && textContains("活动太火爆了").exists()){
          console.log("健康研究室任务火爆，返回首页重新进入活动");
