  ...
  function Run(LauchAPPName, IsSeparation) {
    ...
    while (true) {
      ...
      for (let i = 0; i < taskButtons.length; i++) {
        ...
        if(taskButton.parent().childCount() <= 5){
          continue;
        }
        else{
          taskText = taskButton.parent().child(taskButton.indexInParent() - 1).text();
          if(taskText == ""){
            taskText = taskButton.parent().child(taskButton.indexInParent() - 1).child(0).text();
          }
          taskTitle = taskButton.parent().child(taskButton.indexInParent() - 2).text();
